...
R has powerful (and pretty plotting) functionality in the library ggplot2, which is part of the tidyverse environment. All of tidyverse uses long data. So we need to get the data in the correct structure. Look at the help for gather()
...
benefit_names <- names(benefits)[-1] #create a vector of the benefits using the column names but dropping the date column
benefit_long <- gather(benefits, key = benefit, value = apps, benefit_names)
head(benefit_long)
...
Finally, let's change the axes labels, legend title and background (using themes - look at help for options)
...
benefit_names <- names(benefits)[-1] #create a vector of the benefits using the column names but dropping the date column
#Wide to long
benefit_long <- benefits %>%
  gather(key = benefit, value = apps, benefit_names)
#Get total
benefit_total <- benefit_long %>%
  group_by(date) %>%
  summarise(apps = sum(apps), 
